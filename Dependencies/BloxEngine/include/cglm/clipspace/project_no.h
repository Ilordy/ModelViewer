...
CGLM_INLINE
void
glm_unprojecti_no(vec3 position, mat4 invMat, vec4 vp, vec3 dest) {
  vec4 v;

  v[0] = 2.0f * (position[0] - vp[0]) / vp[2] - 1.0f;
  v[1] = 2.0f * (position[1] - vp[1]) / vp[3] - 1.0f;
  v[2] = 2.0f *  position[2]                  - 1.0f;
  v[3] = 1.0f;
...
CGLM_INLINE
void
glm_project_no(vec3 position, mat4 m, vec4 vp, vec3 dest) {
  CGLM_ALIGN(16) vec4 pos4;

  glm_vec4(position, 1.0f, pos4);
