...
CGLM_INLINE
vec3s
glms_unprojecti_no(vec3s position, mat4s invMat, vec4s vp) {
  vec3s dest;
  glm_unprojecti_no(position.raw, invMat.raw, vp.raw, dest.raw);
  return dest;
}
...
CGLM_INLINE
vec3s
glms_project_no(vec3s position, mat4s m, vec4s vp) {
  vec3s dest;
  glm_project_no(position.raw, m.raw, vp.raw, dest.raw);
  return dest;
}
