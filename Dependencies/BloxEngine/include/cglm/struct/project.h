...
CGLM_INLINE
vec3s
glms_unprojecti(vec3s position, mat4s invMat, vec4s vp) {
  vec3s r;
  glm_unprojecti(position.raw, invMat.raw, vp.raw, r.raw);
  return r;
}
...
CGLM_INLINE
vec3s
glms_unproject(vec3s position, mat4s m, vec4s vp) {
  vec3s r;
  glm_unproject(position.raw, m.raw, vp.raw, r.raw);
  return r;
}
...
CGLM_INLINE
vec3s
glms_project(vec3s position, mat4s m, vec4s vp) {
  vec3s r;
  glm_project(position.raw, m.raw, vp.raw, r.raw);
  return r;
}
